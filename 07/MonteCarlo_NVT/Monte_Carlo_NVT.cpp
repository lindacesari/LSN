...
int main(int argc, char** argv) {
    ...
    if (argc != 2) {
        cout << "Usage: Monte_Carlo_NVT.cpp <phase (solid, liquid, gas)>" << endl;
        return -1;
    }
...
void Input(string phase)
{
    // choose the system phase
    string file_input;
    if (phase == "solid") {
        file_input = "input.solid";
    } else if (phase == "liquid") {
        file_input = "input.liquid";
    } else if (phase == "gas") {
        file_input = "input.gas";
    }
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Gofr.open("output.gofr.0",ios::app);        // blocks
    Gave.open("output.gave.0",ios::app);        // final
    ...
    //g(r)
    for (int i = igofr; i < 102; i++) {
        blk_av[i] /= rho * 4.*M_PI*( pow(bin_size*(double)(i-1.), 3.) - pow(bin_size*(double)(i-2.), 3.) )/3.;
        glob_av[i] += blk_av[i]/blk_norm/(double)npart;
        glob_av2[i] += blk_av[i]/blk_norm/(double)npart * (blk_av[i]/blk_norm/(double)npart);
        err_gdir[i-2] = Error(glob_av[i], glob_av2[i], iblk);
    }

//Potential energy per particle
    Epot << setw(wd) << nstep <<  setw(wd) << stima_pot << setw(wd) << glob_av[iv]/(double)iblk << setw(wd) << err_pot << endl;
//Pressure
    Pres << setw(wd) << nstep <<  setw(wd) << stima_pres << setw(wd) << glob_av[iw]/(double)iblk << setw(wd) << err_press << endl;

//g(r)
    for (int i = igofr; i < igofr+nbins-1; i++)
        Gofr << setw(wd) << iblk <<  setw(wd)<< glob_av[i]/(double)iblk << endl;
    int end = igofr+nbins-1;
    Gofr << setw(wd) << glob_av[end]/(double)iblk << endl;
    
    if (iblk == nblk) {
        for (int i = igofr; i < igofr+nbins; i++)
            Gave << setw(wd) << glob_av[i]/(double)iblk << "," << err_gdir[i-2] << endl;
    }
